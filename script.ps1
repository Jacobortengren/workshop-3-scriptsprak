...
# Läser in JSON-filen som text och konverterar till PowerShell-objekt
...
# Skapar en formaterad sträng med domännamn och exportdatum
$DomainAndDate = @"
...
# Sätter gransdatum 30 dagar bakåt
$gransdatum = (Get-Date).AddDays(-30)
# Filtrerar användare som inte loggat in på över 30 dagar
$inaktivaAnvandare = $adData.users | Where-Object { [datetime]$_.lastLogon -lt $gransdatum }

# Skapar rubrik för rapport
$userReport = @"
---USERS-THAT-HAVENT-LOGGED-IN-FOR-30-DAYS---
"@

# Formaterar användarlistan till tabell
...
# Skapar rubrik för avdelningsrapport
$deptReport = @"
---NUMBER-OF-USERS-PER-DEPARTMENT-----------
"@

# Loopar igenom alla användare och räknar per avdelning
$avdelningCount = @{}
...
# Skapar sträng med avdelningsstatistik
$deptStrings = ""
...
# Rubrik för site-gruppering
$siteReport = @"
---COMPUTERS-GROUPED-BY-SITE----------------
"@

# Grupperar datorer per site och formaterar listan
...
# Exporterar inaktiva användare till CSV-fil
...
# Rubrik för lösenordsålder
$passwordReport = @"
---PASSWORD-AGE-IN-DAYS---------------------
"@

# Loopar igenom användare och räknar antal dagar sedan lösenord sattes
$passwordStrings = ""
$today = Get-Date
...
# Rubrik för topp 10 datorer med längsta tid sedan senaste inloggning
$topCompReport = @"
---TOP 10 COMPUTERS THAT HAVEN'T CHECKED IN------
"@

# Filtrerar bort datorer utan lastLogon och sorterar för att hitta de 10 längst bort
